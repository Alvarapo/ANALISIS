...
public class EntradasExternas extends TablaMedicion{
    ...
    public EntradasExternas(int tipoRegistro , int tipoDato) throws Exception{
        ...
        else if(tipoRegistro == 2) ficherosReferenciados = 1;
        else if(tipoRegistro >= 3) ficherosReferenciados = 2;
        else throw new Exception("El valor de tipo de registro introducido no es válido");
        
        if(tipoDato >= 1 && tipoDato <= 4) tipoDatoFinal = 0;
        else if(tipoDato >= 5 && tipoDato <= 15) tipoDatoFinal = 1;
        else if(tipoDato >= 16) tipoDatoFinal = 2;
